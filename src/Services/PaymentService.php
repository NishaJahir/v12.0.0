...
class PaymentService
{
   ...
   /**
     ...
     * @var redirectPayment
     */
   private $redirectPayment = ['NOVALNET_SOFORT', 'NOVALNET_PAYPAL', 'NOVALNET_IDEAL', 'NOVALNET_EPS', 'NOVALNET_GIROPAY', 'NOVALNET_PRZELEWY24', 'NOVALNET_BANCONTACT', 'NOVALNET_POSTFINANCE_CARD', 'NOVALNET_POSTFINANCE'];
        ...
        if (!empty($orderId)) {
            ...
            foreach($payments as $payment)
            {
                ...
                if($this->paymentHelper->getPaymentKeyByMop($payment->mopId))
                {
                    ...
                    if(in_array($transactionStatus, ['PENDING', 'ON_HOLD', 'SUCCESS']) && ( (in_array($transactionDetails['invoice_type'], ['INVOICE', 'PREPAYMENT']) && ($transactionDetail->amount > $totalCallbackAmount)) || $transactionDetails['paymentName'] == 'novalnet_instalment_invoice') ) {
